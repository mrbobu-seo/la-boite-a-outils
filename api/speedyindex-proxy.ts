...
module.exports = async function (request, response) {
  ...
  try {
    ...
    const contentType = speedyIndexApiResponse.headers.get('content-type') || '';
    if (contentType.toLowerCase().includes('application/json')) {
      ...
      const responseText = await speedyIndexApiResponse.text();
      response.status(speedyIndexApiResponse.status).send(responseText);
    }
